...
class GeckoSessionActionProducer<S>(
    ...
    override fun onCanGoBack(session: GeckoSession?, canGoBack: Boolean) = Unit
    ...
    companion object {
        fun create(context: Context, state: SessionState): GeckoSession {
            return GeckoSession().apply {
                open(GeckoRuntime.getDefault(context.applicationContext))
