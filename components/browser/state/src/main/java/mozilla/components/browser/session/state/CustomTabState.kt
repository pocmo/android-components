 ...
 * create instances.
 */
data class CustomTabState internal constructor(
    val id: String,
    ...
    companion object {
        ...
        fun createFromIntent(intent: SafeIntent): CustomTabState {
            val id = UUID.randomUUID().toString()

            val options = mutableListOf<String>()

            val toolbarColor = if (intent.hasExtra(CustomTabsIntent.EXTRA_TOOLBAR_COLOR)) {
                options.add(TOOLBAR_COLOR_OPTION)
                intent.getIntExtra(CustomTabsIntent.EXTRA_TOOLBAR_COLOR, -1)
            } else {
                null
            }

            val closeButtonIcon = run {
                val icon = intent.getParcelableExtra(CustomTabsIntent.EXTRA_CLOSE_BUTTON_ICON) as? Bitmap
            ...
            val disableUrlbarHiding = !intent.getBooleanExtra(CustomTabsIntent.EXTRA_ENABLE_URLBAR_HIDING, true)
            ...
            val showShareMenuItem = intent.getBooleanExtra(CustomTabsIntent.EXTRA_DEFAULT_SHARE_MENU_ITEM, true)
            ...
            return CustomTabState(id, toolbarColor, closeButtonIcon, disableUrlbarHiding, actionButtonConfig,
        ...
        private fun getActionButtonConfig(intent: SafeIntent): CustomTabActionButtonConfig? {
            ...
            if (intent.hasExtra(CustomTabsIntent.EXTRA_ACTION_BUTTON_BUNDLE)) {
                val abBundle = intent.getBundleExtra(CustomTabsIntent.EXTRA_ACTION_BUTTON_BUNDLE)
                val description = abBundle?.getString(CustomTabsIntent.KEY_DESCRIPTION)
                val icon = abBundle?.getParcelable(CustomTabsIntent.KEY_ICON) as? Bitmap
                val pendingIntent = abBundle?.getParcelable(CustomTabsIntent.KEY_PENDING_INTENT) as? PendingIntent
        ...
        private fun getMenuItems(intent: SafeIntent): List<CustomTabMenuItem> {
            ...
            val menuItemBundles: ArrayList<Parcelable>? = intent.getParcelableArrayListExtra(CustomTabsIntent.EXTRA_MENU_ITEMS)
            menuItemBundles?.forEach {
                if (it is Bundle) {
                    val bundle = SafeBundle(it)
                    val name = bundle.getString(CustomTabsIntent.KEY_MENU_ITEM_TITLE)
                    val pendingIntent = bundle.getParcelable(CustomTabsIntent.KEY_PENDING_INTENT) as? PendingIntent
