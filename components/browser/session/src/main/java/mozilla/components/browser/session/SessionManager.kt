...
class SessionManager(
    ...
    fun add(
        ...
        session.engineSessionHolder.engineObserver?.store = store

        val state = SessionState(url = session.url, id = session.id)

        store?.dispatch(SessionListAction.AddSessionAction(
            state, delegate.selectedSession?.id == session.id
    ...
    fun getOrCreateEngineSession(session: Session = selectedSessionOrThrow): EngineSession {
        val engineSession = delegate.getOrCreateEngineSession(session)
        session.engineSessionHolder.engineObserver?.store = store
    ...
    fun select(session: Session) {
        ...
        store?.dispatch(SessionListAction.SelectSessionAction(session.id))
    }
